...
**Not yet done, still required before this module is considered final** (per the standing instruction that this test node is for iteration, not the final proof): the AD/Samba schema path, an alias-fallback case, the "no LDAP entry"/"LDAP unreachable" fallback paths (VERIFY item 2), dnshelper-present DNS flows (dnshelper is not installed on this node), backup/restore, module removal, and — separately — a full clean-node validation pass once the above is done. Disabling a domain (5.5) is now covered by the concurrency testing above but only for the route-removal/service-stop path; the DNS-record-removal option (`remove_dns`) is still untested since dnshelper isn't installed on this node.

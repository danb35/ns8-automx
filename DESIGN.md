...
**Not yet done, still required before this module is considered final** (per the standing instruction that this test node is for iteration, not the final proof): the AD/Samba schema path, an alias-fallback case, the "no LDAP entry"/"LDAP unreachable" fallback paths (VERIFY item 2), a real backup/restore round-trip (blocked above, needs either a fixed local repository or external backup storage), the `unmanaged` DNS status and dnshelper-install-ordering cases noted above, and — separately — a full clean-node validation pass once the above is done.

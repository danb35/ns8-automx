...
**New, not-yet-investigated bug reported by the user (2026-09-23 planned):** the Domains page in the admin UI did not live-update to reflect a domain being disabled (via `set-domains`) — the row still showed the old enabled state after the action completed. Not yet looked into; likely a Vuex store not being refreshed after the action resolves, or a missing re-fetch of `get-domains`. Planned for the next session alongside AD/Samba testing.
...
Not yet done in this pass (planned next): an AD user with a free-form alias (the static-fallback case, 3.1/3.3), dnshelper + BIND re-setup on this fresh node (needed again post-revert) to re-confirm the DNS flows still work under the AD-bound instance, and the Domains-page live-update bug reported at the end of the previous session.

**Not yet done, still required before this module is considered final** (per the standing instruction that this test node is for iteration, not the final proof): the Domains-page live-update bug, an alias-fallback case (now specifically against AD, having confirmed the primary-address and no-match/unreachable paths above), dnshelper DNS flows re-confirmed against this fresh AD-bound instance, a real backup/restore round-trip (blocked above, needs either a fixed local repository or external backup storage), the `unmanaged` DNS status and dnshelper-install-ordering cases noted earlier, and — separately — a full clean-node validation pass once the above is done.

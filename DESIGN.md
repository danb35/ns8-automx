...
**Backup/restore: initial attempt misdiagnosed as SELinux, then fully completed successfully** (2026-09-23, correcting the entry below from the previous day). The original local `rclone`/`local`-backend attempt (a plain host directory, to avoid needing external WebDAV/S3 infrastructure) failed with `no_read_permission`; at the time this was attributed to "almost certainly an SELinux context issue" without actually confirming it. Revisited properly: `ausearch -m avc` around a fresh repro showed **zero AVC denials** -- the SELinux attribution was wrong. Reading the actual code path (`cluster/actions/add-backup-repository/50update` → node's `validate-backup-destination` → `cluster.backup.run_rclone()`) showed the real mechanism: the read/write check runs as `podman exec rclone-gateway rclone lsd ...` *inside* a long-lived NS8-core container, not as a host process, and that container only has its own dedicated volumes mounted (`/srv/repo`, its cache, and read-only `/etc/rclone` config) -- never an arbitrary host path. Pointing a "local" rclone backend at a directory outside those mounts was never going to work, independent of host-side permissions.

The actual fix: NS8 has a genuine built-in, self-contained backup destination for exactly this case -- `provider: "cluster"`, backed by that same `/srv/repo` volume, served over WebDAV on the cluster's own WireGuard mesh (`webdav:http://<node's wg0 IP>:4694`, confirmed via `cluster.backup.GATEWAY_URL`/`lookup_node_from_webdav_url`). Using it needs no external infrastructure at all. With that repository, ran the **full round-trip and it passed cleanly, no bugs found**: `add-backup` (`instances: ["automx-agent1"]`) → `run-backup` (succeeded in ~13s, 1 snapshot, 4 files matching `state/domains.json`+`state/settings.json`) → `restore-module` into a **new** instance (`automx-agent2`, `replace: false`, following `ns8-dnshelper`'s own safe test pattern of restoring alongside the original rather than overwriting it) → the restored instance came up with the service active, the domain's `enabled: true` and settings (`http2https`, `display_names`) correctly recovered from the backup, routes correctly recreated under its own new module identity, DNS correctly reported `not_permitted` (a new module identity genuinely needs its own dnshelper grant -- not something restore should silently inherit, and it didn't), and `GET /mail/config-v1.1.xml?emailaddress=dan@adtest.example` against the restored instance returned the fully correct LDAP-resolved response. The original `automx-agent1` instance was confirmed unaffected throughout. This directly confirms `state-include.conf` and `restore-module/50reconcile` are both correct as designed, not just "re-reviewed by hand" as the previous entry here said.
...
**`unmanaged` DNS status and the dnshelper-coverage-ordering case, both confirmed together, no bugs found** (2026-09-23): added a second mail domain (`other.example`, via `mail1/add-domain`) deliberately outside dnshelper's existing `adtest.example` zone, and enabled it in automx. It correctly showed `managed: false`, every record `status: "unmanaged"`, while `adtest.example` was completely unaffected -- confirming per-domain DNS status is independent. `apply-dns` correctly refused to act on it (`zone_unmanaged`), matching 6.3's contract. Then, **without touching automx's state at all** (no disable/re-enable, no `set-domains` call), added a second zone to the same BIND instance for `other.example`, registered it with dnshelper (`add-zone`), and re-checked: the status live-transitioned to `managed: true, allowed: false, status: "not_permitted"` purely from dnshelper learning about the zone. Granting the access policy rule (still with zero automx-side changes) transitioned it again to `allowed: true, status: "missing"`. This directly confirms the DESIGN.md 6.2 implementation choice (`get-domains`/`check-dns` compute DNS status live on every call, no cache, no `service-dnshelper-changed` handler needed) actually delivers what the "dnshelper installed/zone covered after the module was already configured" scenario in 9.2's test matrix requires. Ran `apply-dns` create for `other.example` to close it out; both domains ended at `status: "ok"`.

**Sixth real-node pass, 2026-09-23: the final clean-node validation, completed with no surprises.** The user reverted the test VM to the same clean NS8-core-only snapshot one more time, specifically so this last pass would prove the *whole* install-from-scratch experience works end to end on the current, fully-fixed codebase, uncontaminated by anything discovered or worked around in earlier passes. Ran the complete stack in one continuous sequence -- Samba AD provisioning, mail module bound to it, automx (image rebuilt fresh from the current committed tree; the agent image hashed identically to the prior build, confirming no drift), the real autoconfig endpoint (correct on the first request, no restart-timing hiccup this time), BIND + dnshelper (the SELinux `dns_port_t` TCP label applied proactively before first start, so that detour didn't recur either), the DNS create flow, and a full backup/restore round-trip into a second instance -- and every single step succeeded cleanly on the first attempt, matching exactly what each individual pass had already established. No new bugs found. This closes DESIGN.md 9.2's "mandatory... not an optional nice-to-have" real-node integration requirement: every functional item in its test matrix has now been verified at least once against real infrastructure, and this final pass confirms none of those individual verifications were accidentally relying on leftover state from the debugging that found them.

**Status: this module's core functionality is real-node verified and considered done for v1**, modulo the acknowledged out-of-scope items in section 2 and the "later phases" in section 12. Remaining open items are narrower and lower-stakes than "does it work": `tests/integration/`'s automated scenario matrix is written but still not run end-to-end as a re-runnable suite (9.2 -- everything in its matrix has been exercised manually across these six passes, but not yet through that harness itself); client testing against real Thunderbird/Outlook/Apple Mail (9.3) remains manual and outstanding; and `http2https`'s default (VERIFY item 6) still awaits that same client testing.

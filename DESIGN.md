...
**Implementation status (2026-09-22):** 9.1 is done and passing (`tests/unit/`, 60 tests; `.github/workflows/test-unit.yml`; the `build-images.sh` smoke check; `tests/automx.robot` updated for the real action schemas). 9.2's harness (`tests/integration/`) is written, following dnshelper's own structure, but has **not been run against a real node** — no node was available in this session. It covers a representative core path through the matrix below (install through removal, both LDAP outcomes, both dnshelper states) rather than literally every row; treat it as a solid starting scaffold to extend and actually execute, not as evidence the flows it describes work. One spot check while writing it already paid off: the first draft asserted the wrong rendered `<username>` for a successful LDAP match (the full address, not `${login}`'s bare value) — caught by re-deriving it from `render-automx-conf`'s own template rather than trusting the draft.

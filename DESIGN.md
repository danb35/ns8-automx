...
Not yet done in this pass (planned next): dnshelper + BIND re-setup on this fresh node (needed again post-revert) to re-confirm the DNS flows still work under the AD-bound instance, and the Domains-page live-update bug reported at the end of the previous session.

**Not yet done, still required before this module is considered final** (per the standing instruction that this test node is for iteration, not the final proof): the Domains-page live-update bug, dnshelper DNS flows re-confirmed against this fresh AD-bound instance, a real backup/restore round-trip (blocked above, needs either a fixed local repository or external backup storage), the `unmanaged` DNS status and dnshelper-install-ordering cases noted earlier, and — separately — a full clean-node validation pass once the above is done.
